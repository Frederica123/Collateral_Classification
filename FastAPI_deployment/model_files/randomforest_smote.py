...
loaded_rf = joblib.load("model_joblibs/" + config['randomforest_smote_joblib'])
# original_df = pd.read_csv('before_smote_standard_token_from_6.csv',index_col = 0)
description = pd.read_json(config['description_rf_smote'], encoding = 'utf-8-sig')
##test_datat = {'colateral_text':"DELL COMPUTER"}
...
def rf_predict(test_data):
    ...
    # p_dict = [str(i) for i in list(loaded_rf.predict_proba(input_)[0])]
    p_dict = {}
